...
type FrozenDB struct {
	// Core file resources
	file fileio.DBFile // DBFile interface for file operations
	...
	finder finder.Finder // Finder interface for locating rows by UUID key
}
...
func NewFrozenDB(path string, mode string, strategy FinderStrategy) (*FrozenDB, error) {
	...
	dbFile, err := fileio.NewDBFile(path, mode)
	if err != nil {
		return nil, err
	}
	...
	header, err := fileio.ValidateDatabaseFile(dbFile)
	if err != nil {
		cleanupErr = err
		return nil, err
	}
	...
	var finderImpl finder.Finder
	switch strategy {
	case FinderStrategySimple:
		finderImpl, err = finder.NewSimpleFinder(dbFile, rowSize)
	case FinderStrategyInMemory:
		finderImpl, err = finder.NewInMemoryFinder(dbFile, rowSize)
	case FinderStrategyBinarySearch:
		finderImpl, err = finder.NewBinarySearchFinder(dbFile, rowSize)
	}
	if err != nil {
		cleanupErr = err
		return nil, err
...
func (db *FrozenDB) recoverTransaction() error {
	...
	if remainder != 0 {
		...
		writeChan := make(chan fileio.Data, 100)
...
func (db *FrozenDB) Get(key uuid.UUID, value any) error {
	...
	var endRowUnion fields.RowUnion
