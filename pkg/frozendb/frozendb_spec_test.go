	...
	tests := []struct {
		...
		},
		{
			...
			setupFile: func(t *testing.T, path string) {
				file, _ := os.Create(path)
				defer file.Close()
				h, _ := NewHeader(HEADER_SIGNATURE, 1, 1024, 5000)
...
// Helper function to count open file descriptors (stub for testing)
...
func Test_S_004_FR_001_ValidateMethodExists(t *testing.T) {
	...
	header, _ := NewHeader("fDB", 1, 1024, 5000)
	err := header.Validate()
...
func Test_S_004_FR_002_DirectInitRequiresValidation(t *testing.T) {
	...
	invalidHeader, err := NewHeader("XXX", 1, 1024, 5000)
	if err == nil {
		t.Error("Invalid header should fail during construction")
	}
	if invalidHeader != nil {
...
func Test_S_004_FR_005_ValidateMethodIsIdempotent(t *testing.T) {
	...
	invalidHeader, _ := NewHeader("XXX", 1, 1024, 5000)
	if invalidHeader != nil {
	...
	tests := []struct {
		...
		},
		{
			...
			setupFile: func(t *testing.T, path string, rowSize int) error {
				...
				defer file.Close()
				h, _ := NewHeader(HEADER_SIGNATURE, 1, rowSize, 5000)
				header, _ := h.MarshalText()
				file.Write(header)
				return nil
			},
