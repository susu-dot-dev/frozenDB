...
func (cfg *CreateConfig) Validate() error {
	...
	header, err := NewHeader(HEADER_SIGNATURE, 1, cfg.rowSize, cfg.skewMs)
	if err != nil {
		return err
	}

	// Ensure header was created successfully (defensive check)
	if header == nil {
		return NewInvalidInputError("failed to create header", nil)
	}
...
func Create(config CreateConfig) error {
	...
	header, err := NewHeader(HEADER_SIGNATURE, 1, config.rowSize, config.skewMs)
	if err != nil {
		return err
	}
