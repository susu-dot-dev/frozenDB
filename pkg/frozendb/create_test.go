...
func TestGenerateHeader(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			header, err := NewHeader(HEADER_SIGNATURE, 1, tt.rowSize, tt.skewMs)
			if err != nil && !tt.wantErr {
				t.Errorf("NewHeader() unexpected error = %v", err)
				return
			}
			if err == nil && tt.wantErr {
				t.Errorf("NewHeader() expected error but got none")
				return
			}
			if tt.wantErr {
				return // Skip marshal test if we expect error
			}
			headerBytes, err := header.MarshalText()
			if err != nil {
				t.Errorf("Header.MarshalText() error = %v", err)
				return
			}
