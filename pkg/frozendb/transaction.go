...
func (tx *Transaction) Commit() error {
	...
	if len(tx.rows) == 0 && tx.last.GetState() == PartialDataRowWithStartControl {
		...
		// Create and validate NullRow with timestamp-aware UUID
		nullRow, err := NewNullRow(tx.Header.GetRowSize(), maxTimestamp)
		if err != nil {
			return NewInvalidActionError("failed to create NullRow", err)
		}
